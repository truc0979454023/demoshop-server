...
const userCtrl = {
  register: async (req, res) => {
    try {
      ...
      if (user)
        return res.status(400).json({ msg: "The email already sxists." });

      if (password.length < 6)
        return res
          .status(400)
          .json({ msg: "Pass is at least 6 characters long." });
  ...
  updatePasword: async (req, res) => {
    try {
      ...
      if (!email || !password || !new_password)
        return res
          .status(400)
          .json({ msg: "Please add password and new password." });
      ...
      if (new_password === password)
        return res.status(400).json({
          msg: "Please change the new password that does not match the old password.",
        });
